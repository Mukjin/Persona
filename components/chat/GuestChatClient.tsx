...
function makeStarter(name: string) {
  return [
    `안녕하세요. ${name}의 세계관을 한 문장으로 소개해줘.`,
    `${name}의 말투로 자기소개해줘.`,
    `${name}가 Spring Boot를 설명하면 어떤 식으로 말해줄까?`
  ];
}
...
export function GuestChatClient({
  ...
  const starters = useMemo(() => makeStarter(persona.name), [persona.name]);
  const [interpreterMode, setInterpreterMode] = useState(false);

  const transport = useMemo(
    () =>
      new DefaultChatTransport({
        api: "/api/public-chat/stream",
        body: { slug: persona.slug, interpreterMode }
      }),
    [persona.slug, interpreterMode],
  );
      ...
      <div className="surface-card-strong rounded-[34px] p-6 sm:p-7">
        <div className="flex items-start justify-between gap-4 border-b border-black/5 pb-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="soft-pill">Guest Chat</div>
          </div>
        </div>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {starters.map((starter) => (
            <button
              ...
              className="rounded-full border border-white/75 bg-white/72 px-3 py-2 text-xs text-[rgb(var(--muted))] shadow-[inset_0_1px_0_rgba(255,255,255,0.92)] hover:-translate-y-0.5 hover:border-[rgba(99,102,241,0.22)] hover:text-[rgb(var(--text))]"
            >
              {starter}
            </button>
          ))}
        ...
        <div className="mt-6 grid gap-4">
          {messages.length === 0 ? (
            <div className="rounded-[26px] border border-white/78 bg-white/70 px-5 py-5 text-sm leading-8 text-[rgb(var(--muted))]">
              질문을 보내면 {persona.name}의 성격과 말투가 반영된 답변이 바로 이어집니다.
            </div>
          ) : (
